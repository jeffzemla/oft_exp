...
#       + Demo trial order:                                   #
#          - hard, medium, easy, no coherence, full coherence #
#       + All trials are 'right'                              #
#                                                             #
###############################################################
...
screen = VisionEgg.Core.Screen(fullscreen=1)          # get_default_screen crashes my laptop
...
def getState(t):
    global fixOn, dotsOn, changeCoh, changeDir, keyPressed, fixOn_start, dotsOn_start, rt, trialnum
...
def setCoherence(t):
    ...
    if changeCoh:
        changeCoh=0
        if trialnum == 1:
            cohLevel = coherence[0]
        if trialnum == 2:
            cohLevel = coherence[1]
        if trialnum == 3:
            cohLevel = coherence[2]
        if trialnum == 4:
            cohLevel = 0
        if trialnum == 5:
            cohLevel = 1
